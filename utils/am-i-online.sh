...
function aio_check-dns() {
  ...
  echo -e "${pre_success} DNS Cloudflare Online${post_string}" || \
  echo -e "${pre_failure} DNS Cloudflare Offline${post_string}"
}

function aio_check-dns-adguard1() {
  : >/dev/tcp/192.168.1.8/53 > /dev/null && \
  echo -e "${pre_success} DNS AdGuard 1 Online${post_string}" || \
  echo -e "${pre_failure} DNS AdGuard 1 Offline${post_string}"
}

function aio_check-dns-adguard2() {
  : >/dev/tcp/192.168.1.10/53 > /dev/null && \
  echo -e "${pre_success} DNS AdGuard 2 Online${post_string}" || \
  echo -e "${pre_failure} DNS AdGuard 2 Offline${post_string}"
}
...
function aio_start() {
  ...
  echo -e ${line}
  aio_check-dns
  aio_check-dns-adguard1
  aio_check-dns-adguard2
  aio_ping-gateway
